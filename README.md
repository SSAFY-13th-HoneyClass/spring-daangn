...
- 액세스 토큰, 리프레시 토큰 방식이 있음
- 클라이언트가 API 요청 시, 이 토큰을 **HTTP Header**에 넣어 서버에 전달
