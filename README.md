...
### 세션 (Session)
* 로그인 시 서버에 세션 정보를 저장하고, 클라이언트는 세션 ID를 쿠키에 담아 보냄.
* 서버가 상태를 유지해야 하며, 서버 메모리에 부담이 갈 수 있음.

### 쿠키 (Cookie)
* 인증 정보를 쿠키에 저장하고 클라이언트에서 자동 전송되도록 설정.
* 보안 설정(HTTPOnly, Secure 등)을 하지 않으면 탈취 위험이 있음.

### JWT (Json Web Token)
* 사용자 정보를 암호화된 토큰에 담아 클라이언트에 저장, 서버는 상태를 저장하지 않음.
* 확장성과 성능이 좋지만, 토큰 탈취 시 위험하므로 리프레시 토큰 관리가 중요.

### OAuth 2.0
* 외부 서비스(Google, Kakao 등)로부터 사용자 인증을 위임받는 방식.
* 사용자는 직접 비밀번호를 입력하지 않고, 인증/인가를 분리할 수 있음.
